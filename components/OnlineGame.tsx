                        ...
                        try {
                            ...
                            if (snapshot.exists()) {
                                ...
                                for (const roomId in allRooms) {
                                    ...
                                    if (!roomId || /[\u0000-\u001F\u007F.#$\[\]\/]/.test(roomId) || typeof allRooms[roomId] !== 'object' || allRooms[roomId] === null) {
                                        ...
                                        continue;
                                                            ...
                                                            const hasConnectedHumanPlayers = players.some((p: any) => p && p.id && !p.id.startsWith('BOT-') && p.connectionStatus === 'connected');
                                ...
                                if (roomsToDelete.length > 0) {
                                    const deletionPromises = roomsToDelete.map(id => db.ref(`rooms/${id}`).remove());
                                    await Promise.all(deletionPromises);
                                    console.log(`${roomsToDelete.length} empty/stale rooms deleted.`);
                                }
                            }
                        } catch (e) {
