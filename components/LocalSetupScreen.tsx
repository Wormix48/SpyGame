...
const PlayerListItem: React.FC<PlayerListItemProps> = React.memo(({ player, index, editingIndex, editingName, onEdit, onSaveEdit, setEditingName, onRemove, onAvatarClick, onDragStart, onDragEnter, onDragEnd, onDragOver }) => {
    return (
        <div
        ...
        >
            <div className="flex items-center gap-2 flex-grow min-w-0">
              ...
              <button onClick={() => onAvatarClick(player.id)} className="flex-shrink-0 relative group">
...
export const LocalSetupScreen: React.FC<LocalSetupScreenProps> = ({ onGameStart }) => {  const [players, setPlayers] = useState<PlayerProfile[]>([
    { id: generateId(), name: 'Игрок 1', avatar: RANDOM_AVATARS[0] },
    { id: generateId(), name: 'Игрок 2', avatar: RANDOM_AVATARS[1] },
    { id: generateId(), name: 'Игрок 3', avatar: RANDOM_AVATARS[2] },
  ]);
  ...
  const [selectedPlayerIdForAvatar, setSelectedPlayerIdForAvatar] = useState<string | null>(null);
  ...
  const handleAddPlayer = () => {
    ...
    if (trimmedName && !players.some(p => p.name === trimmedName)) {
      ...
      setPlayers([...players, { id: generateId(), name: trimmedName, avatar: randomAvatar }]);
      setNewPlayerName('');
    }
  };
  ...
  const handleAvatarClick = (playerId: string) => {
    setSelectedPlayerIdForAvatar(playerId);
    setIsEmojiPickerOpen(true);
  };

  const handleEmojiSelect = (emoji: string) => {
    if (selectedPlayerIdForAvatar) {
      setPlayers(currentPlayers => {
        const newPlayers = currentPlayers.map(p => 
          p.id === selectedPlayerIdForAvatar ? { ...p, avatar: emoji } : p
        );
        return newPlayers;
      });
    }
    setIsEmojiPickerOpen(false);
    setSelectedPlayerIdForAvatar(null);
  };
          ...
          <form onSubmit={handleGameStartClick} className="w-full md:w-1/2 space-y-6">
            ...
            <div>
              ...
              <div className="flex justify-center bg-slate-700 rounded-lg p-1 items-center gap-1">
                ...
                <div className="relative w-1/2">
                    <button type="button" onClick={handleAiSourceClick} className={`w-full py-2 rounded-md font-semibold transition-colors ${questionSource === 'ai' ? 'bg-cyan-500 text-slate-900' : 'text-slate-300 hover:bg-slate-600'}`}>ИИ Генерация</button>
                    {questionSource === 'ai' && (<button type="button" onClick={() => setIsApiModalOpen(true)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full bg-cyan-700/50 hover:bg-cyan-600" aria-label="Изменить API ключ"><PencilIcon className="h-4 w-4 text-white" /></button>)}
                </div>
              </div>
            </div>
            <div>
            ...
            <div>
              <label htmlFor="showQuestionToSpyToggle" className="flex items-center justify-between text-lg font-medium text-slate-300 cursor-pointer">
                  <span className="flex items-center gap-1 text-left">Показывать вопрос шпиону</span>
                  <div className="relative inline-flex items-center">
