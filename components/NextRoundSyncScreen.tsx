        ...
        return (
            <div className="flex flex-col items-center justify-center text-center animate-fade-in min-h-[450px]">
                ...
                <div className="w-full max-w-sm space-y-2 bg-slate-700 p-3 rounded-lg mb-6">
                    {activePlayers.map(p => (
                        <div key={p.id} className={`flex items-center justify-between p-2 rounded-md transition-colors ${p.readyForNextRound || p.connectionStatus === 'disconnected' ? 'bg-green-600/50' : 'bg-slate-800'}`}>
                           ...
                           <div className="flex items-center gap-2">
                                {p.readyForNextRound || p.connectionStatus === 'disconnected' ? (
